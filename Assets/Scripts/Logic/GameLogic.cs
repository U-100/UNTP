	...
	public class GameLogic : IGameLogic
	{
		...
		private void SpawnPlayerCharacters(IGameBoard board)
		{
			for (int playerIndex = 0; playerIndex < board.players.count; playerIndex++)
			{
				...
				{
					int3 proposedSpawnPosition = int3(this._random.NextInt(0, (int)board.settings.playerSettings.visionSize), 0, this._random.NextInt(0, (int)board.settings.playerSettings.visionSize));
